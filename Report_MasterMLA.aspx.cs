...
{
    public partial class Report_MasterMLA : System.Web.UI.Page
    {
        ...
        public void Khasdar()
        {
            ...
            //SqlDataAdapter sda = new SqlDataAdapter("select DISTINCT [LekhaShirshName] from [BudgetMasterMLA]", con);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ddlReportType.Items.Add(dr["AmdaracheName"].ToString());
            }
            ddlReportType.Items.Add("संपूर्ण");
        }
        public void Year()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select DISTINCT Arthsankalpiyyear from MLAProvision", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
        ...
        public void LekhasirshAll()
        {
            ...
            {
                SqlDataAdapter sda = new SqlDataAdapter("select BR.[AmdaracheName] as 'AdmdarName',count(BR.PageNo)as 'no of works',BR.Upvibhag as 'Division',sum(RP.ManjurAmt)as 'AA cost',sum(RP.MarchEndingExpn)as 'Expend Up to March',sum(RP.UrvaritAmt)as 'Balance Cost',sum(RP.Takunone)as 'Budget Provision',sum(RP.AkunAnudan)as 'Relessed',sum(RP.Magilkharch)as 'Expend up to sept',sum(RP.Magni)as 'Demand',sum(RP.[Apr]) as 'Apr',sum(RP.[May]) as 'May',sum(RP.[Jun]) as 'Jun',sum(RP.[Jul]) as 'Jul',sum(RP.[Aug]) as 'Aug',sum(RP.[Sep]) as 'Sep',sum(RP.[Oct]) as 'Oct',sum(RP.[Nov]) as 'Nov',sum(RP.[Dec]) as 'Dec',sum(RP.[Jan]) as 'Jan',sum(RP.[Feb]) as 'Feb',sum(RP.[Mar]) as 'Mar',sum(Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec) as 'Total' from BudgetMasterMLA as BR join MLAProvision as RP on BR.WorkId=RP.WorkId where RP.[Arthsankalpiyyear]='" + ddlArthsankalpiyYear.SelectedItem.ToString() + "' and BR.AmdaracheName=N'" + ddlReportType.SelectedItem.Text + "'  group by BR.[AmdaracheName], BR.Upvibhag", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                GridView1.DataSource = dt;
                GridView1.DataBind();
            ...
            {
                lblPrintAmdar.Text = "आमदार : " + ddlReportType.SelectedItem.ToString();
            }
        ...
        protected void Print(object sender, EventArgs e)
        {
            try
            {
                ...
                GridView1.DataBind();
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key1", "<script>MakeStaticHeader('" + GridView1.ClientID + "', auto, 100 , 100 ,true); </script>", false);
            }
            catch (Exception)
            {
            }
        ...
        protected void BtnExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "MLA_Abstract_Report.xls"));
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (ddlReportType.SelectedItem.Text != "संपूर्ण")
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cntc FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + ddlReportType.SelectedItem.ToString() + "'  AND Sadyasthiti=N'Completed' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cntp FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + ddlReportType.SelectedItem.ToString() + "'  AND Sadyasthiti=N'Processing' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cnts FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + ddlReportType.SelectedItem.ToString() + "'  AND Sadyasthiti=N'Tender Stage' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as astar FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + ddlReportType.SelectedItem.ToString() + "'  AND Sadyasthiti=N'Estimated Stage' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                        lblAstar.Text = dr["astar"].ToString();
            ...
            else
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    Label lblC = (Label)e.Row.FindControl("lblC");
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    Label lblAmdarName = (Label)e.Row.FindControl("lblAmdarName");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cntc FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + lblAmdarName.Text + "'  AND Sadyasthiti=N'Completed' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    Label lblAmdarName = (Label)e.Row.FindControl("lblAmdarName");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cntp FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + lblAmdarName.Text + "' AND Sadyasthiti=N'Processing' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    Label lblAmdarName = (Label)e.Row.FindControl("lblAmdarName");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as cnts FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + lblAmdarName.Text + "' AND Sadyasthiti=N'Tender Stage' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    Label lbllupvibhag = (Label)e.Row.FindControl("lbllupvibhag");
                    Label lblAmdarName = (Label)e.Row.FindControl("lblAmdarName");
                    SqlDataAdapter sda = new SqlDataAdapter("SELECT Sadyasthiti,isnull(COUNT([Sadyasthiti]),0) as astar FROM BudgetMasterMLA WHERE Upvibhag=N'" + lbllupvibhag.Text + "' and AmdaracheName=N'" + lblAmdarName.Text + "' AND Sadyasthiti=N'Estimated Stage' group by Sadyasthiti", con);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    foreach (DataRow dr in dt.Rows)
                    {
            ...
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key2", "<script>MakeStaticHeader('" + GridView1.ClientID + "', auto, 100 , 100 ,true); </script>", false);
