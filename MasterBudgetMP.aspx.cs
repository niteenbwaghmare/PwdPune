...
{
    public partial class MasterBudgetMP : System.Web.UI.Page, IHttpHandler, IRequiresSessionState
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["id"] != null)
                {
                    ...
                    GetID();
                    akunanudan();
                    BindAll_Lekha_Vibhag_VarishtType("MP");
        ...
        public void GetID()
        {
            ...
            {
                int i = 1;
                string select = "select top 1 Akrmank from BudgetMasterMP order by Akrmank desc";
        ...
        public void auditdateset()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Arthsankalpiyyear from MPProvision where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            if (dt.Rows.Count != 0)
            {
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("select Arthsankalpiyyear from BudgetMasterMP where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
        ...
        public void workidsearch()
        {
            try
            {
                ...
                dt = ObjsqlQueryOrCon.Feach_Master_Data("MP", txtWorkID.Text.Trim());
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        ddltyp.SelectedItem.Text = dr["Type"].ToString();
        ...
        {
            return ObjsqlQueryOrCon.GetCompletionListOfWorkID(prefixText, "MP");
        }
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select count(*) from MPProvision where Arthsankalpiyyear=N'" + AuditDate.Text.Trim() + "' and Workid=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
        ...
        {
            DataTable dt = new DataTable();
            dt = ObjsqlQueryOrCon.FeachProvisionData("MP", AuditDate.Text.Trim(), txtWorkID.Text.Trim());
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                ddlakun3.SelectedIndex = -1;
                txtakun3.Text = 0.ToString();
                ddlakun4.SelectedIndex = -1;
        ...
        protected void BtnSav_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (conMDB.State != ConnectionState.Open)
                    conMDB.Open();
                SqlDataAdapter sda = new SqlDataAdapter("select Count(*) from BudgetMasterMP WHERE WorkId='" + txtWorkID.Text.Trim() + "'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (cmd.ExecuteNonQuery() > 0 && cmdMDB.ExecuteNonQuery() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            Server.Transfer("MasterBudgetMP.aspx", false);
