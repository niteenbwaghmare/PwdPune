...
{
    public partial class NewBudgetMaster : System.Web.UI.Page, IHttpHandler, IRequiresSessionState
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["id"] != null)
                {
                    ...
                    akunanudan();
                    BindAll_Lekha_Vibhag_VarishtType("Building");
                    Jilha();
        ...
        public void GetID()
        {
            ...
            {
                int i = 1;
                string select = "select top 1 Akrmank from BudgetMasterBuilding order by Akrmank desc";
        ...
        public void auditdateset()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Arthsankalpiyyear from BuildingProvision where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            if (dt.Rows.Count != 0)
            {
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("select Arthsankalpiyyear from BudgetMasterBuilding where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
        ...
        public void workidsearch()
        {
            try
            {
                ...
                DataTable dt = new DataTable();
                dt = ObjsqlQueryOrCon.Feach_Master_Data("Building", txtWorkID.Text.Trim());
        ...
        {
            return ObjsqlQueryOrCon.GetCompletionListOfWorkID(prefixText, "Building");
        }
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select count(*) from BuildingProvision where Arthsankalpiyyear=N'" + AuditDate.Text.Trim() + "' and Workid=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            TextBox[] txtIds = { txtmudatvadhdinak, Billpayment, txtcost, txtmarchakherkharch, txtakun1, txtakun2, txtakun3, txtakun4, txtjan, txtfeb, txtmar, txtapr, txtmay, txtjun, txtjul, txtaug, txtsep, txtoct, txtnov, txtdec, txtchalukharch, txtmagilkharch, txtmagni, txtVidyutprama, txtVidyutvitarit, txtitarkhrch, txtpahnikramank, txtdnyapane, txtpahnimudye, txtshera, txtkamachenav, txtkamachavav, txtprashaskiykramank, txtprashaskiydinak, txtprashaskiykimat, txttantrikkramank, txttantarikdinak, txttantarikkimat, txtNividaKramank, txtNividaKimat, txtkaryarambhaadesh, txtkaryarambhdinak, txtkamachimudat, txtkampurndinak };
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                else
                {
                    for (int i = 0; i < txtIds.Length; i++)
                    {
                        txtIds[i].Enabled = false;
                    }

                    for (int i = 0; i < ddlIds.Length; i++)
                    {
                        ddlIds[i].Enabled = false;
                    }
                }
            }
        }
        //This Method Fetching Data from Provision Table using StoredProcedure
        public void fetchdate()
        {

            DataTable dt = new DataTable();
            dt = ObjsqlQueryOrCon.FeachProvisionData("Building", AuditDate.Text.Trim(), txtWorkID.Text.Trim());
        ...
        protected void txtsecurity_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                TextBox[] txtIds = { txtmudatvadhdinak, Billpayment, txtcost, txtmarchakherkharch, txtakun1, txtakun2, txtakun3, txtakun4, txtjan, txtfeb, txtmar, txtapr, txtmay, txtjun, txtjul, txtaug, txtsep, txtoct, txtnov, txtdec, txtchalukharch, txtmagilkharch, txtmagni, txtVidyutprama, txtVidyutvitarit, txtitarkhrch, txtpahnikramank, txtdnyapane, txtpahnimudye, txtshera };
                DropDownList[] ddlIds = { ddlakun1, ddlakun2, ddlakun3, ddlakun4, ddlchalukharch, ddlmagilmonth, ddlsadyasthiti };
        ...
        protected void BtnSav_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtnividashakhaPassword.Text = ConvertDigits(txtnividashakhaPassword.Text);
                ...
                if (conMDB.State != ConnectionState.Open)
                    conMDB.Open();
                SqlDataAdapter sda = new SqlDataAdapter("select Count(*) from BudgetMasterBuilding WHERE WorkId='" + txtWorkID.Text.Trim() + "'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (cmd.ExecuteNonQuery() > 0 && cmdMDB.ExecuteNonQuery() > 0)
                    {
                        ...
                        SMSobj.shakhaabhyantaname = ddlabhiyanta.SelectedItem.ToString();
                        SMSobj.shakhaabhyantamobile = txtabhiyantamobile.Text.Trim();
                        SMSobj.upabhyantaname = ddlupabhiyanta.SelectedItem.ToString();
                        SMSobj.upabhyantamobile = txtupabhiyantamobile.Text.Trim();
                        SMSobj.kamachename = txtkamachenav.Text.Trim();
                        SMSobj.thekedarname = ddlThekedarName.SelectedItem.ToString();
                        ...
                        else
                        {
                            ...
                            Server.Transfer("MasterBudgetBuilding.aspx", false);
                        }
        ...
        protected void txtnividashakhaPassword_TextChanged(object sender, EventArgs e)
        {

            if (txtnividashakhaPassword.Text == FormPassword.ToString())
            {
                //NividaShakha
                ddlThekedarName.Enabled = txtNividaKramank.Enabled = true;
