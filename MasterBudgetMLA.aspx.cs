...
{
    public partial class MasterBudgetMLA : System.Web.UI.Page, IHttpHandler, IRequiresSessionState
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["id"] != null)
                {
                    ...
                    GetID();
                    akunanudan();
                    BindAll_Lekha_Vibhag_VarishtType("MLA");
                    Jilha();
        ...
        public void GetID()
        {
            ...
            {
                int i = 1;
                string select = "select top 1 Akrmank from BudgetMasterMLA order by Akrmank desc";
        ...
        public void auditdateset()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Arthsankalpiyyear from MLAProvision where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            if (dt.Rows.Count != 0)
            {
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("select Arthsankalpiyyear from BudgetMasterMLA where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
        ...
        public void workidsearch()
        {
            try
            {
                ...
                dt = ObjsqlQueryOrCon.Feach_Master_Data("MLA", txtWorkID.Text.Trim());
        ...
        {
            return ObjsqlQueryOrCon.GetCompletionListOfWorkID(prefixText, "MLA");
        }
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select count(*) from MLAProvision where Arthsankalpiyyear=N'" + AuditDate.Text.Trim() + "' and Workid=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
        ...
        {
            DataTable dt = new DataTable();
            dt = ObjsqlQueryOrCon.FeachProvisionData("MLA", AuditDate.Text.Trim(), txtWorkID.Text.Trim());
        ...
        protected void BtnSav_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ddlsadyasthiti.Text = ConvertDigits(ddlsadyasthiti.Text);
                ...
                if (conMDB.State != ConnectionState.Open)
                    conMDB.Open();
                SqlDataAdapter sda = new SqlDataAdapter("select Count(*) from BudgetMasterMLA WHERE WorkId='" + txtWorkID.Text.Trim() + "'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (cmd.ExecuteNonQuery() > 0 && cmdMDB.ExecuteNonQuery() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            Server.Transfer("MasterBudgetMLA.aspx", false);
