...
{
    public partial class MasterCRF1 : System.Web.UI.Page, IHttpHandler, IRequiresSessionState
    {
       ...
       static SqlQueryOrConnection ObjsqlQueryOrCon = new SqlQueryOrConnection();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["id"] != null)
                {
                    ...
                    GetID();
                    akunanudan();
                    BindAll_Lekha_Vibhag_VarishtType("Nabard");
                    Jilha();
        ...
        public void GetID()
        {
            ...
            {
                int i = 1;
                string select = "select top 1 Akrmank from BudgetMasterNABARD order by Akrmank desc";
        ...
        public void auditdateset()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select Arthsankalpiyyear from NABARDProvision where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            if (dt.Rows.Count != 0)
            {
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("select Arthsankalpiyyear from BudgetMasterNABARD where WorkID=N'" + txtWorkID.Text.Trim() + "'", con);
        ...
        public void lekhashirsh()
        {
            try
            {
                ...
                ddllekhashirsh.Items.Add(" ");
                SqlDataAdapter sda = new SqlDataAdapter("select code from SettingLekhaShirsh where Type=N'Nabard'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                foreach (DataRow dr in dt.Rows)
                {
        ...
        public void workidsearch()
        {
            try
            {
                ...
                dt = ObjsqlQueryOrCon.Feach_Master_Data("Nabard", txtWorkID.Text.Trim());
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        ddlRIDFNo.SelectedValue = dr["RDF_SrNo"].ToString();
                        ddlRIDFNo.SelectedItem.Text = dr["RDF_NO"].ToString();
                        txtpicno.Text = dr["PIC_NO"].ToString();
                        txtpcr.Text = dr["PCR"].ToString();
                        txtroadno.Text = dr["Road_No"].ToString();
                        txtlengthroad.Text = dr["LengthRoad"].ToString();
                        Roadtype.SelectedItem.Text = dr["RoadType"].ToString();
                        txtwbmikm.Text = dr["WBMI_km"].ToString();
                        txtwbmiikm.Text = dr["WBMII_km"].ToString();
                        txtwbmiiikm.Text = dr["WBMIII_km"].ToString();
                        txtbbmkm.Text = dr["BBM_km"].ToString();
                        txtcarpetkm.Text = dr["Carpet_km"].ToString();
                        txtsurfacekm.Text = dr["Surface_km"].ToString();
                        txtcdworksno.Text = dr["CD_Works_No"].ToString();
                ...
                else
                {
                    ...
                    ddlRIDFNo.SelectedIndex = 0;
                    txtpicno.Text = "";
                    txtpcr.Text = "";
                    txtroadno.Text = "";
                    txtlengthroad.Text = "";
                    Roadtype.SelectedIndex = 0;
                    txtwbmikm.Text = 0.ToString();
                    txtwbmiikm.Text = 0.ToString();
                    txtwbmiiikm.Text = 0.ToString();
                    txtbbmkm.Text = 0.ToString();
                    txtcarpetkm.Text = 0.ToString();
                    txtsurfacekm.Text = 0.ToString();
                    txtcdworksno.Text = "";
        ...
        {
            return ObjsqlQueryOrCon.GetCompletionListOfWorkID(prefixText, "Nabard");
        }
        ...
        public void totalkharch()
        {
            ...
            if (txtmarchakherkharch.Text != "")
            {
                txtaikunkharch.Text = (Convert.ToDecimal(txtmarchakherkharch.Text) + Convert.ToDecimal(txtvarshbharatilkharch.Text)).ToString();
            }
        }
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select count(*) from NABARDProvision where Arthsankalpiyyear=N'" + AuditDate.Text.Trim() + "' and Workid=N'" + txtWorkID.Text.Trim() + "'", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            foreach (DataRow dr in dt.Rows)
            {
                if (dr[0].ToString() == 0.ToString())
                {
                    ...
                    ddlRIDFNo.Enabled = true;
                    txtpicno.Enabled = true;
                    txtpcr.Enabled = true;
                    txtroadno.Enabled = true;
                    txtlengthroad.Enabled = true;
                    Roadtype.Enabled = true;
                    txtwbmikm.Enabled = true;
                    txtwbmiikm.Enabled = true;
                    txtwbmiiikm.Enabled = true;
                    txtsurfacekm.Enabled = true;
                    txtbbmkm.Enabled = true;
                    txtcarpetkm.Enabled = true;
                    txtcdworksno.Enabled = true;
                ...
                else
                {
                    ...
                    //Work completed in kms
                    ddlRIDFNo.Enabled = false;
                    txtpicno.Enabled = false;
                    txtpcr.Enabled = false;
                    txtroadno.Enabled = false;
                    txtlengthroad.Enabled = false;
                    Roadtype.Enabled = false;
                    txtwbmikm.Enabled = false;
                    txtwbmiikm.Enabled = false;
                    txtwbmiiikm.Enabled = false;
                    txtsurfacekm.Enabled = false;
                    txtbbmkm.Enabled = false;
                    txtcarpetkm.Enabled = false;
                    txtcdworksno.Enabled = false;
                }
            }
        }
        ...
        public void fetchdate()
        {
            ...
            DataTable dt = new DataTable();
            dt = ObjsqlQueryOrCon.FeachProvisionData("Nabard", AuditDate.Text.Trim(), txtWorkID.Text.Trim());
            ...
            if (dt.Rows.Count != 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    ddlakun1.Text = dr["DTakunone"].ToString();
                    txtakun1.Text = dr["Takunone"].ToString();
                    ddlakun2.Text = dr["DTakuntwo"].ToString();
                    txtakun2.Text = dr["Takuntwo"].ToString();
                    ddlakun3.Text = dr["DTakunthree"].ToString();
                    txtakun3.Text = dr["Takunthree"].ToString();
                    ddlakun4.Text = dr["DTakunfour"].ToString();
                    ...
                    ddlchalukharch.Text = dr["Chalumonth"].ToString();
                    txtchalukharch.Text = dr["Chalukharch"].ToString();
                    ddlmagilmonth.Text = dr["Magilmonth"].ToString();
        ...
        protected void BtnSav_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtpicno.Text = ConvertDigits(txtpicno.Text);
                txtroadno.Text = ConvertDigits(txtroadno.Text);
                txtwbmikm.Text = ConvertDigits(txtwbmikm.Text);
                txtwbmiikm.Text = ConvertDigits(txtwbmiikm.Text);
                txtwbmiiikm.Text = ConvertDigits(txtwbmiiikm.Text);
                txtsurfacekm.Text = ConvertDigits(txtsurfacekm.Text);
                txtbbmkm.Text = ConvertDigits(txtbbmkm.Text);
                txtcarpetkm.Text = ConvertDigits(txtcarpetkm.Text);
                txtcdworksno.Text = ConvertDigits(txtcdworksno.Text);
                ...
                if (conMDB.State != ConnectionState.Open)
                    conMDB.Open();
                SqlDataAdapter sda = new SqlDataAdapter("select Count(*) from BudgetMasterNABARD WHERE WorkId=N'" + txtWorkID.Text.Trim() + "'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr[0].ToString() == 0.ToString())
                    {
                        string strMasterInsertQuery = "INSERT INTO [BudgetMasterNABARD] ([WorkId],[Arthsankalpiyyear],[Akrmank],[Type],[Dist] ,[Taluka],[ArthsankalpiyBab],[Upvibhag],[Lekhashirsh],[LekhaShirshName],[SubType],[ShakhaAbhyantaName],[ShakhaAbhiyantMobile],[UpabhyantaName],[UpAbhiyantaMobile],[KhasdaracheName],[AmdaracheName],[KamacheName] ,[Kamachavav],[PrashaskiyKramank],[PrashaskiyDate],[PrashaskiyAmt],[TrantrikKrmank],[TrantrikDate] ,[TrantrikAmt],[ThekedaarName],[ThekedarMobile] ,[NividaKrmank] ,[NividaAmt],[karyarambhadesh],[NividaDate],[kamachiMudat],[KamPurnDate],[RDF_NO],[PIC_NO],[PCR],[Road_No],[LengthRoad],[RoadType],[WBMI_km],[WBMII_km],[WBMIII_km],[BBM_km],[Carpet_km],[Surface_km],[CD_Works_No],[Sadyasthiti] ,[Pahanikramank] ,[PahaniMudye] ,[Shera] ,[Img1],[Img2],[Img3],[RDF_SrNo],[SubDivision]) VALUES(N'" + txtWorkID.Text.Trim() + "',N'" + txtarthsankalpiyyear.Text + "',N'" + Convert.ToInt32(lblId.Text) + "',N'Nabard',N'" + ddldist.SelectedItem.Text + "',N'" + ddltaluka.SelectedItem.Text + "',N'" + txtarthsankalpiybab.Text + "',N'" + ddlupvibhag.Text + "',N'" + ddllekhashirsh.SelectedItem.Text + "',N'" + lblLekhaName.Text + "',N'" + ddlsubtype.SelectedItem.Text + "',N'" + ddlabhiyanta.SelectedItem.Text + "',N'" + txtabhiyantamobile.Text + "',N'" + ddlupabhiyanta.SelectedItem.Text + "',N'" + txtupabhiyantamobile.Text + "',N'" + ddlkhasdarachename.SelectedItem.Text + "',N'" + ddlaamdarachename.SelectedItem.Text + "',N'" + txtkamachenav.Text + "',N'" + txtkamachavav.Text + "',N'" + txtprashaskiykramank.Text + "',N'" + txtprashaskiydinak.Text + "',N'" + Convert.ToDecimal(txtprashaskiykimat.Text) + "',N'" + txttantrikkramank.Text + "',N'" + txttantarikdinak.Text + "',N'" + Convert.ToDecimal(txttantarikkimat.Text) + "',N'" + ddlThekedarName.SelectedItem.Text + "',N'" + lblThekedarMobNo.Text + "',N'" + txtNividaKramank.Text + "',N'" + txtNividaKimat.Text + "',N'" + txtkaryarambhaadesh.Text + "',N'" + txtkaryarambhdinak.Text + "',N'" + txtkamachimudat.Text + "',N'" + txtkampurndinak.Text + "',N'" + ddlRIDFNo.SelectedItem.Text + "',N'" + txtpicno.Text + "',N'" + txtpcr.Text + "',N'" + txtroadno.Text + "',N'" + txtlengthroad.Text + "',N'" + Roadtype.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtwbmikm.Text) + "',N'" + Convert.ToDecimal(txtwbmiikm.Text) + "',N'" + Convert.ToDecimal(txtwbmiiikm.Text) + "',N'" + Convert.ToDecimal(txtbbmkm.Text) + "',N'" + Convert.ToDecimal(txtcarpetkm.Text) + "',N'" + Convert.ToDecimal(txtsurfacekm.Text) + "',N'" + txtcdworksno.Text + "',N'" + ddlsadyasthiti.SelectedItem.Text + "',N'" + txtpahnikramank.Text + "',N'" + txtpahnimudye.Text + "',N'" + txtshera.Text + "',N'" + a + "',N'" + b + "',N'" + c + "'," + ddlRIDFNo.SelectedItem.Value.ToString() + ",'PuneEast')";
                        string strProvisionInsertQuery = "INSERT INTO [NABARDProvision] ([WorkId],[Arthsankalpiyyear],[MudatVadhiDate],[DeyakachiSadyasthiti],[ManjurAmt],[MarchEndingExpn],[UrvaritAmt],[DTakunone],[Takunone],[DTakuntwo],[Takuntwo],[DTakunthree],[Takunthree],[DTakunfour],[Takunfour],[Tartud],[Jan],[Feb],[Mar],[Apr],[May],[Jun],[Jul],[Aug],[Sep],[Oct],[Nov],[Dec],[AkunAnudan],[Chalumonth],[Chalukharch],[Magilmonth],[Magilkharch],[Magni],[VarshbharatilKharch],[AikunKharch],[SubDivision]) VALUES('" + txtWorkID.Text.Trim() + "','" + txtarthsankalpiyyear.Text + "',N'" + txtmudatvadhdinak.Text + "',N'" + Billpayment.Text + "',N'" + Convert.ToDecimal(txtcost.Text) + "',N'" + Convert.ToDecimal(txtmarchakherkharch.Text) + "',N'" + Convert.ToDecimal(txturvaritamt.Text) + "',N'" + ddlakun1.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun1.Text) + "', N'" + ddlakun2.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun2.Text) + "',N'" + ddlakun3.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun3.Text) + "',N'" + ddlakun4.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun4.Text) + "',N'" + Convert.ToDecimal(txttartud.Text) + "',N'" + Convert.ToDecimal(txtjan.Text) + "',N'" + Convert.ToDecimal(txtfeb.Text) + "',N'" + Convert.ToDecimal(txtmar.Text) + "',N'" + Convert.ToDecimal(txtapr.Text) + "',N'" + Convert.ToDecimal(txtmay.Text) + "',N'" + Convert.ToDecimal(txtjun.Text) + "',N'" + Convert.ToDecimal(txtjul.Text) + "',N'" + Convert.ToDecimal(txtaug.Text) + "',N'" + Convert.ToDecimal(txtsep.Text) + "',N'" + Convert.ToDecimal(txtoct.Text) + "',N'" + Convert.ToDecimal(txtnov.Text) + "',N'" + Convert.ToDecimal(txtdec.Text) + "',N'" + Convert.ToDecimal(txtaikunanudan.Text) + "',N'" + ddlchalukharch.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtchalukharch.Text) + "',N'" + ddlmagilmonth.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtmagilkharch.Text) + "',N'" + Convert.ToDecimal(txtmagni.Text) + "',N'" + Convert.ToDecimal(txtvarshbharatilkharch.Text) + "',N'" + Convert.ToDecimal(txtaikunkharch.Text) + "','PuneEast')";
                    ...
                    {
                        SqlDataAdapter sda1 = new SqlDataAdapter("select Count(*) from NABARDProvision where Arthsankalpiyyear=N'" + AuditDate.Text + "' and WorkId=N'" + txtWorkID.Text.Trim() + "'", con);
                        DataTable dt1 = new DataTable();
                        sda1.Fill(dt1);
                        foreach (DataRow dr1 in dt1.Rows)
                        {
                            string strMasterUpdateQuery = "UPDATE BudgetMasterNABARD SET [Dist]=N'" + ddldist.SelectedItem.Text + "',[Taluka]=N'" + ddltaluka.SelectedItem.Text + "',[ArthsankalpiyBab]=N'" + txtarthsankalpiybab.Text + "',[Upvibhag]=N'" + ddlupvibhag.SelectedItem.Text + "',[SubType]=N'" + ddlsubtype.SelectedItem.Text + "',[LekhaShirsh]=N'" + ddllekhashirsh.SelectedItem.Text + "',[LekhaShirshName]=N'" + lblLekhaName.Text + "',[ShakhaAbhyantaName]=N'" + ddlabhiyanta.SelectedItem.Text + "',[ShakhaAbhiyantMobile]=N'" + txtabhiyantamobile.Text + "',[UpabhyantaName]=N'" + ddlupabhiyanta.SelectedItem.Text + "',[UpAbhiyantaMobile]=N'" + txtupabhiyantamobile.Text + "',[KhasdaracheName]=N'" + ddlkhasdarachename.SelectedItem.Text + "',[AmdaracheName]=N'" + ddlaamdarachename.SelectedItem.Text + "',[KamacheName]=N'" + txtkamachenav.Text + "',[Kamachavav]=N'" + txtkamachavav.Text + "',[PrashaskiyKramank]=N'" + txtprashaskiykramank.Text + "',[PrashaskiyDate]=N'" + txtprashaskiydinak.Text + "',[PrashaskiyAmt]=N'" + txtprashaskiykimat.Text + "',[TrantrikKrmank]=N'" + txttantrikkramank.Text + "',[TrantrikDate]=N'" + txttantarikdinak.Text + "',[TrantrikAmt]=N'" + Convert.ToDecimal(txttantarikkimat.Text) + "',[ThekedaarName]=N'" + ddlThekedarName.SelectedItem.Text + "',[ThekedarMobile]=N'" + lblThekedarMobNo.Text + "',[NividaKrmank]=N'" + txtNividaKramank.Text + "',[NividaAmt]=N'" + txtNividaKimat.Text + "',[karyarambhadesh]=N'" + txtkaryarambhaadesh.Text + "',[NividaDate]=N'" + txtkaryarambhdinak.Text + "',[kamachiMudat]=N'" + txtkamachimudat.Text + "',[KamPurnDate]=N'" + txtkampurndinak.Text + "',[RDF_NO]=N'" + ddlRIDFNo.SelectedItem.Text + "',[PIC_NO]=N'" + txtpicno.Text + "',[PCR]=N'" + txtpcr.SelectedItem.Text + "',[Road_No]=N'" + txtroadno.Text + "',[LengthRoad]=N'" + txtlengthroad.Text + "',[RoadType]=N'" + Roadtype.SelectedItem.Text + "',[WBMI_km]=N'" + Convert.ToDecimal(txtwbmikm.Text) + "',[WBMII_km]=N'" + Convert.ToDecimal(txtwbmiikm.Text) + "',[WBMIII_km]=N'" + Convert.ToDecimal(txtwbmiiikm.Text) + "',[BBM_km]=N'" + Convert.ToDecimal(txtbbmkm.Text) + "',[Carpet_km]=N'" + Convert.ToDecimal(txtcarpetkm.Text) + "',[Surface_km]=N'" + Convert.ToDecimal(txtsurfacekm.Text) + "',[CD_Works_No]=N'" + txtcdworksno.Text + "',[Sadyasthiti]=N'" + ddlsadyasthiti.SelectedItem.Text + "',[Pahanikramank]=N'" + txtpahnikramank.Text + "',[PahaniMudye]=N'" + txtpahnimudye.Text + "',[Shera]=N'" + txtshera.Text + "',[RDF_SrNo]=" + ddlRIDFNo.SelectedItem.Value.ToString() + " WHERE WorkId=N'" + txtWorkID.Text.Trim() + "' and [SubDivision]='PuneEast'";
                            cmd = new SqlCommand(strMasterUpdateQuery, con);
                            cmdMDB = new SqlCommand(strMasterUpdateQuery, conMDB);
                           
                            if (dr1[0].ToString() == 0.ToString())
                            {
                                string strMasterInsertQuery = "INSERT INTO [NABARDProvision] ([WorkId],[Arthsankalpiyyear],[MudatVadhiDate],[DeyakachiSadyasthiti],[ManjurAmt],[MarchEndingExpn],[UrvaritAmt],[DTakunone],[Takunone],[DTakuntwo],[Takuntwo],[DTakunthree],[Takunthree],[DTakunfour],[Takunfour],[Tartud],[Jan],[Feb],[Mar],[Apr],[May],[Jun],[Jul],[Aug],[Sep],[Oct],[Nov],[Dec],[AkunAnudan],[Chalumonth],[Chalukharch],[Magilmonth],[Magilkharch],[Magni],[VarshbharatilKharch],[AikunKharch],[SubDivision]) VALUES('" + txtWorkID.Text.Trim() + "','" + txtarthsankalpiyyear.Text + "',N'" + txtmudatvadhdinak.Text + "',N'" + Billpayment.Text + "',N'" + Convert.ToDecimal(txtcost.Text) + "',N'" + Convert.ToDecimal(txtmarchakherkharch.Text) + "',N'" + Convert.ToDecimal(txturvaritamt.Text) + "',N'" + ddlakun1.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun1.Text) + "', N'" + ddlakun2.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun2.Text) + "',N'" + ddlakun3.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun3.Text) + "',N'" + ddlakun4.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtakun4.Text) + "',N'" + Convert.ToDecimal(txttartud.Text) + "',N'" + Convert.ToDecimal(txtjan.Text) + "',N'" + Convert.ToDecimal(txtfeb.Text) + "',N'" + Convert.ToDecimal(txtmar.Text) + "',N'" + Convert.ToDecimal(txtapr.Text) + "',N'" + Convert.ToDecimal(txtmay.Text) + "',N'" + Convert.ToDecimal(txtjun.Text) + "',N'" + Convert.ToDecimal(txtjul.Text) + "',N'" + Convert.ToDecimal(txtaug.Text) + "',N'" + Convert.ToDecimal(txtsep.Text) + "',N'" + Convert.ToDecimal(txtoct.Text) + "',N'" + Convert.ToDecimal(txtnov.Text) + "',N'" + Convert.ToDecimal(txtdec.Text) + "',N'" + Convert.ToDecimal(txtaikunanudan.Text) + "',N'" + ddlchalukharch.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtchalukharch.Text) + "',N'" + ddlmagilmonth.SelectedItem.Text + "',N'" + Convert.ToDecimal(txtmagilkharch.Text) + "',N'" + Convert.ToDecimal(txtmagni.Text) + "',N'" + Convert.ToDecimal(txtvarshbharatilkharch.Text) + "',N'" + Convert.ToDecimal(txtaikunkharch.Text) + "','PuneEast')";
                                cmd1 = new SqlCommand(strMasterInsertQuery, con);
                                cmdMDB1 = new SqlCommand(strMasterInsertQuery, conMDB);                             
                            }
                            else
                            {
                                strMasterUpdateQuery = "UPDATE [NABARDProvision] SET [MudatVadhiDate]=N'" + txtmudatvadhdinak.Text + "',[DeyakachiSadyasthiti]=N'" + Billpayment.Text + "',[ManjurAmt]=N'" + txtcost.Text + "',[MarchEndingExpn]=N'" + txtmarchakherkharch.Text + "',[UrvaritAmt]=N'" + txturvaritamt.Text + "',[DTakunone]=N'" + ddlakun1.SelectedItem.Text + "',[Takunone]=N'" + txtakun1.Text + "',[DTakuntwo]=N'" + ddlakun2.SelectedItem.Text + "',[Takuntwo]=N'" + txtakun2.Text + "',[DTakunthree]=N'" + ddlakun3.SelectedItem.Text + "',[Takunthree]=N'" + txtakun3.Text + "',[DTakunfour]=N'" + ddlakun4.Text + "',[Takunfour]=N'" + txtakun4.Text + "',[Tartud]=N'" + txttartud.Text + "',[Jan]=N'" + txtjan.Text + "',[Feb]=N'" + txtfeb.Text + "',[Mar]=N'" + txtmar.Text + "',[Apr]=N'" + txtapr.Text + "',[May]=N'" + txtmay.Text + "',[Jun]=N'" + txtjun.Text + "',[Jul]=N'" + txtjul.Text + "',[Aug]=N'" + txtaug.Text + "',[Sep]=N'" + txtsep.Text + "',[Oct]=N'" + txtoct.Text + "',[Nov]=N'" + txtnov.Text + "',[Dec]=N'" + txtdec.Text + "',[AkunAnudan]=N'" + txtaikunanudan.Text + "',[Chalumonth]=N'" + ddlchalukharch.SelectedItem.Text + "',[Chalukharch]=N'" + txtchalukharch.Text + "',[Magilmonth]=N'" + ddlmagilmonth.SelectedItem.Text + "',[Magilkharch]=N'" + txtmagilkharch.Text + "',[Magni]=N'" + txtmagni.Text + "',[VarshbharatilKharch]=N'" + txtvarshbharatilkharch.Text + "',[AikunKharch]=N'" + txtaikunkharch.Text + "' where Arthsankalpiyyear=N'" + AuditDate.Text.Trim() + "' and WorkId=N'" + txtWorkID.Text.Trim() + "' and [SubDivision]='PuneEast'";
                    ...
                    if (cmd.ExecuteNonQuery() > 0 && cmdMDB.ExecuteNonQuery() > 0)
                    {
                        ...
                        else
                        {
                            ...
                            Server.Transfer("MasterBudgetNABARD.aspx", false);
        ...
        protected void txtWorkCompletedPass_TextChanged(object sender, EventArgs e)
        {
            if (txtWorkCompletedPass.Text == FormPassword.ToString())
            {
                //Work completed in kms
                ddlRIDFNo.Enabled = true;
                txtpicno.Enabled = true;
