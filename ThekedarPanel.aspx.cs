...
{
    public partial class ThekedarPanel : System.Web.UI.Page
    {
        ...
        protected void BindColChart()
        {
            ...
            strcmd = "select  (select  count(*) from BudgetMasterBuilding where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Building ,(select  count(*) from  BudgetMasterCRF where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as CRF ,(select  count(*) from BudgetMasterAunty where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName  )as Annuity,(select  count(*) from BudgetMasterDepositFund where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Deposit, (select  count(*) from  BudgetMasterDPDC where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as DPDC , (select  count(*) from  BudgetMasterGAT_A where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Gat_A , (select  count(*) from  BudgetMasterGAT_D where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Gat_D , (select  count(*) from  BudgetMasterGAT_FBC where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName  ) as Gat_BCF, (select  count(*) from  BudgetMasterMLA where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName  )as MLA , (select  count(*) from  BudgetMasterMP where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName ) as MP, (select  count(*) from  BudgetMasterNABARD where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Nabard , (select  count(*) from  BudgetMasterRoad where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as Road , (select  count(*) from  BudgetMasterNonResidentialBuilding where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as NRB2059 , (select  count(*) from  BudgetMasterResidentialBuilding where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as RB2216  , (select count(*) from [BudgetMaster2515] where ThekedaarName=N'" + userName.ToString() + "' group by ThekedaarName )as GramVikas ";
        ...
        protected void gvParentGrid_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                string strQuery = "SELECT a.[Sadyasthiti]as 'Work Status', Count(a.[Sadyasthiti])as'Total Work',sum(cast(a.[PrashaskiyAmt] as decimal(10,2))) as 'AA cost Rs in lakhs',sum(cast(a.[TrantrikAmt]as decimal(10,2)))as 'Technical Sanction Cost Rs in Lakh',sum(cast(b.[Tartud]as decimal(10,2))) as 'Total Provision Rs in Lakh',sum(cast(b.[AikunKharch]as decimal(10,2))) as 'Total Expense Rs in Lakh' FROM " + FromTbl + " a full outer join " + provisionTbl + "  b on a.workid=b.workid where a.[Sadyasthiti]!='' and ThekedaarName=N'" + userName.ToString() + "' and b.Arthsankalpiyyear='" + ddlYear.SelectedItem.ToString() + "' GROUP BY a.[Sadyasthiti]";
